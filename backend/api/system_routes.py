...
async def get_status(state=Depends(get_app_state)):
    ...
    status["qwen3_asr_preview_max_line_length"] = int(getattr(state.asr_engine, "qwen3_preview_max_line_length", -1) or -1)
...
async def update_orchestrator_config(payload: OrchestratorConfigPayload, state=Depends(get_app_state)):
    ...
    old_qwen3_preview_max_line_length = int(getattr(state.asr_engine, "qwen3_preview_max_line_length", -1) or -1)
