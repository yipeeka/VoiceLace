...
class ApiSmokeTest(unittest.TestCase):
    ...
    def test_system_status_contains_backends(self) -> None:
        ...
        self.assertIn("asr_vocal_separation", body)
        self.assertIn("asr_vocal_separation_model", body)
        self.assertIn("asr_vocal_separation_repo_dir_exists", body)
        self.assertIn("asr_vocal_separation_enabled", body.get("config", {}))
        self.assertIn("asr_vocal_separation_model", body.get("config", {}))
        self.assertIn("asr_vocal_separation_repo_dir", body.get("config", {}))
        self.assertIn("asr_vocal_separation_device", body.get("config", {}))
    ...
    def test_update_orchestrator_llm_params(self) -> None:
        ...
        try:
            ...
            self.assertEqual(body["gemini_api_key"], "********")
            self.assertTrue(body["gemini_api_key_configured"])
            ...
            masked_payload = dict(body)
            masked_payload["openai_model"] = "gpt-test-updated"
            response = self.client.put("/api/v1/system/orchestrator/config", json=masked_payload)
            self.assertEqual(response.status_code, 200)
            body = response.json()
            self.assertEqual(body["openai_api_key"], "********")
            self.assertEqual(body["gemini_api_key"], "********")
            ...
            self.assertEqual(persisted["openai_api_key"], "sk-openai")
            self.assertEqual(persisted["gemini_api_key"], "gemini-key")
            self.assertEqual(persisted["openai_model"], "gpt-test-updated")
        finally:
    ...
    def test_transcribe_missing_file_returns_404(self) -> None:
        missing_audio_path = self.app_state.settings.voices_dir / "not_exists.wav"
        response = self.client.post(
            "/api/v1/voices/transcribe",
            json={"audio_path": str(missing_audio_path)},
        )
        self.assertEqual(response.status_code, 404)
        body = response.json()
