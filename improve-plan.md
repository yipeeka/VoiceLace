...
- `P2-1` 部分完成：状态面板已展示解释器、`llama_cpp` 可用性、模块路径、fallback 状态

### 暂缓

- `P0-1` 文件浏览接口白名单化（按当前要求暂不实施）

### 待完成

- `P2-2` 增补回归测试矩阵（后端 + 前端联动覆盖，待本机执行一键验收）
...
- 后端：`.\.venv\Scripts\python.exe -m unittest backend.tests.test_api_smoke backend.tests.test_task_flows backend.tests.test_persistence` -> `Ran 18 tests ... OK`
- 前端：`npm test` -> `pass 15 / fail 0`
