...
export default function ParseQcPage({ onNavigate }) {
  ...
  const [isIssueListOpen, setIsIssueListOpen] = useState(true);
      ...
      <GlassCard>
        <div className="sectionHeader">
          ...
          <Button
            variant="ghost"
            size="sm"
            icon={isIssueListOpen ? ChevronUp : ChevronDown}
            aria-expanded={isIssueListOpen}
            onClick={() => setIsIssueListOpen((prev) => !prev)}
          >
            {isIssueListOpen ? "收起" : "展开"}
          </Button>
        </div>
        {isIssueListOpen ? (
          !issues.length ? (
